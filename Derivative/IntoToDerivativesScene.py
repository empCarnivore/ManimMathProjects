...
class IntroToDerivativesScene(FormatedScene):
    def construct(self):
        ...
        start_point.add_updater(lambda o: o.become(Dot(axes.i2gp(x_tracker.get_value(), curve),radius=DEFAULT_DOT_RADIUS*zoom_tracker.get_value()/START_ZOOM, color=Y0_COLOR)))
        # endregion

        # region end point
        end_point = Dot(axes.i2gp(x_tracker.get_value() + change_x_tracker.get_value()*zoom_tracker.get_value()/START_ZOOM, curve), color=Y1_COLOR)
        end_point.add_updater(lambda o: o.become(
            Dot(axes.i2gp(x_tracker.get_value() + change_x_tracker.get_value()*zoom_tracker.get_value()/START_ZOOM, curve),radius=DEFAULT_DOT_RADIUS*zoom_tracker.get_value()/START_ZOOM, color=Y1_COLOR)))
        # endregion
        ...
        x_change_line = Line(axes.i2gp(x_tracker.get_value(), h_line),
                             axes.i2gp(x_tracker.get_value() + change_x_tracker.get_value()*zoom_tracker.get_value()/START_ZOOM, h_line), color=DX_COLOR)
        x_change_line.add_updater(lambda o: o.become(Line(axes.i2gp(x_tracker.get_value(), h_line), axes.i2gp(
            x_tracker.get_value() + change_x_tracker.get_value()*zoom_tracker.get_value()/START_ZOOM, h_line), color=DX_COLOR)))
        ...
        y_change_line = Line(axes.i2gp(x_tracker.get_value() + change_x_tracker.get_value()*zoom_tracker.get_value()/START_ZOOM, h_line),
                             axes.i2gp(x_tracker.get_value() + change_x_tracker.get_value()*zoom_tracker.get_value()/START_ZOOM, curve), color=DY_COLOR)
