...
class TransformByIndexMap(AnimationGroup):
    def __init__(
        ...
        self.show_indices = show_indices or len(index_map) == 0
    ...
    def process_entry(self, A, B, entry):
        assert len(entry) in [2, 3], "Invalid index_map entry: " + str(entry)
        if self.printing: print("index map entry: ", entry)
        ...
        if not entry[0] and not entry[1]:
            ...
            raise ValueError("Invalid index_map entry: " + str(entry))

    def process_empty_entry(self):
        if self.printing:
            print("Empty index_map entry.")
    ...
    def show_indices_animations(self, A, B, index_label_height, A_index_labels_color, B_index_labels_color):
        ...
        if MANIM_TYPE == 'GL':
            index_labels_A = index_labels(A, label_height=index_label_height).set_color(
                A_index_labels_color).set_z_index(10).set_stroke(color=BLACK, width=3)
            index_labels_B = index_labels(B, label_height=index_label_height).set_color(
                B_index_labels_color).set_z_index(10).set_stroke(color=BLACK, width=3)
        elif MANIM_TYPE == 'CE':
            index_labels_A = index_labels(A, label_height=index_label_height, color=A_index_labels_color,
                                          background_stroke_width=3).set_z_index(10)
            index_labels_B = index_labels(B, label_height=index_label_height, color=B_index_labels_color,
                                          background_stroke_width=3).set_z_index(10)
